...
# Section 3: Exploratory Data Analysis (Per-Building Profiles with Save)
...
# - Profiled data (e.g., mean consumption tables) will be saved to 'profiling/data/'.
...
ELECTRICITY_CSV = 'cleaned_electricity.csv'
AREAS_CSV = 'cleaned_areas.csv'
# WEATHER_CSV = 'cleaned_weather.csv' # Load if weather correlations are added

# Define output directories for plots and potentially data
PROFILING_DIR = 'profiling'
PLOT_OUTPUT_DIR = os.path.join(PROFILING_DIR, 'plots')
DATA_OUTPUT_DIR = os.path.join(PROFILING_DIR, 'data') # For saving profiled data tables
...
def load_cleaned_data():
    ...
    try:
        ...
        # weather = pd.read_csv(WEATHER_CSV) # Uncomment if weather data is needed
    except FileNotFoundError as e:
        print(f"Error: One or more cleaned CSV files not found. Please run data_cleaning.py first.")
        print(f"Details: {e}")
        return None, None, None 

    # Set datetime index for electricity data
...
def plot_electricity_consumption_over_time(elec_df, area_map_dict):
    ...
    plt.figure(figsize=(20, 10), dpi=FIGURE_DPI) # Increased size
    ...
    for col in plot_columns:
        if col in elec_df:
            ...
            plt.plot(elec_df.index, elec_df[col], label=lbl, lw=1, alpha=0.8) # Slightly thicker line

    plt.title("Electricity Consumption Over the Year by Building", fontsize=18)
    plt.xlabel("Date", fontsize=14)
    plt.ylabel("kWh", fontsize=14)
    plt.legend(fontsize='medium', ncol=max(1, len(plot_columns) // 8)) # Adjusted legend
...
def plot_hourly_profile(elec_df, area_map_dict):
    ...
    print("Plotting and saving data for average hourly consumption profile (per building)...")
    ...
    if not building_cols:
        print("No building/numeric columns found for hourly profile.")
        return

    num_buildings = len(building_cols)
    if num_buildings == 0:
        print("No building columns to plot for hourly profile.")
        return
    ...
    for i, building in enumerate(building_cols):
        if building in df_for_grouping:
            ...
            all_hourly_profiles_data[building] = hourly_profile_building # Store data table
    ...
    plot_filename = os.path.join(PLOT_OUTPUT_DIR, "hourly_consumption_profile_per_building.png")
    plt.savefig(plot_filename)
    print(f"Plot saved: {plot_filename}")
    plt.close()
    
    # Save the hourly profile data tables
    ensure_output_dir(DATA_OUTPUT_DIR) # Ensure data directory exists
    for bldg, profile_data in all_hourly_profiles_data.items():
        if not profile_data.empty:
            data_filename = os.path.join(DATA_OUTPUT_DIR, f"hourly_profile_{bldg}.csv")
            profile_data.to_csv(data_filename, header=[f'{bldg}_avg_kWh']) # Save with a descriptive header
            print(f"Data saved: {data_filename}")
    print(f"Hourly profile data tables saved to {DATA_OUTPUT_DIR}")
...
def plot_per_building_profiles(elec_df, area_map_dict, profile_type):
    ...
    all_profile_data_tables_for_type = {} # To store data tables for this profile type
    ...
    for i, building_col in enumerate(building_cols):
        ...
        profile_table = pd.DataFrame() # Initialize as empty for data saving
        ...
        if profile_type == 'weekly':
            ...
            if not grouped.empty and building_col in grouped:
                profile_table = grouped[building_col].unstack(level=0) # dayofweek becomes columns
            ...
            if not grouped.empty and building_col in grouped:
                profile_table = grouped[building_col].unstack(level=0) # month becomes columns
                if not profile_table.empty:
                    profile_table.columns = [month_names_map[m] for m in sorted(profile_table.columns)]
            ...
            if not grouped.empty and building_col in grouped:
                profile_table = grouped[building_col].unstack(level=0) # Season becomes columns
                if not profile_table.empty:
                    profile_table = profile_table.reindex(columns=season_order) 
            palette = sns.color_palette("coolwarm", 4)
            legend_title = "Season"
        else:
            continue 
        
        all_profile_data_tables_for_type[building_col] = profile_table # Store the data table

        if profile_table.empty:
            ax.text(0.5, 0.5, 'No data', ha='center', va='center', transform=ax.transAxes, fontsize=10)
        else:
            for j, col_name in enumerate(profile_table.columns): # Iterate through days/months/seasons
    ...
    # Save the profile data tables for this type
    ensure_output_dir(DATA_OUTPUT_DIR) # Ensure data directory exists
    for bldg, data_table in all_profile_data_tables_for_type.items():
        if not data_table.empty:
            data_filename = os.path.join(DATA_OUTPUT_DIR, f"{profile_type}_profile_{bldg}.csv")
            data_table.to_csv(data_filename) # Index (hour) will be saved by default
            print(f"Data saved: {data_filename}")
    print(f"{profile_type.capitalize()} profile data tables saved to {DATA_OUTPUT_DIR}")


def main():
    """Main function to run EDA."""
    ensure_output_dir(PLOT_OUTPUT_DIR) # Create plot output directory
    ensure_output_dir(DATA_OUTPUT_DIR) # Create data output directory
